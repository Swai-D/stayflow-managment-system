...
import { useSuppliers, useCreateSupplier } from '@/hooks/useStore'
...
// ── Mock data — TODO: replace with useSuppliers() ───────────────────────────
const MOCK_SUPPLIERS: (Supplier & { itemCount?: number; totalOrders?: number; totalValue?: number; lastOrder?: string })[] = [
  {
    id:'s1', name:'Morogoro General Supplies', phone:'+255712000001',
    email:'supplies@mgs.co.tz', address:'Morogoro Town, Lumumba Street',
    paymentTerms:'30 days net', notes:'Reliable supplier — delivers within 24hrs',
    isActive:true, itemCount:8, totalOrders:5, totalValue:620000,
    lastOrder: new Date(Date.now()-86400000).toISOString(),
  },
  {
    id:'s2', name:'Tanzania Breweries Ltd', phone:'+255712000002',
    email:undefined, address:'Dar es Salaam (Morogoro branch)',
    paymentTerms:'14 days net', notes:'Minimum order: 2 crates per item',
    isActive:true, itemCount:2, totalOrders:3, totalValue:367200,
    lastOrder: new Date(Date.now()-259200000).toISOString(),
  },
  {
    id:'s3', name:'Karibu Textiles', phone:'+255712000003',
    email:'karibu.textiles@gmail.com', address:'Morogoro Industrial Area',
    paymentTerms:'Cash on delivery', notes:'Quality linen supplier. Custom sizes available.',
    isActive:true, itemCount:3, totalOrders:2, totalValue:680000,
...
// ── Supplier Form Modal ──────────────────────────────────────────────────────
function SupplierFormModal({ supplier, onClose }: { supplier?: typeof MOCK_SUPPLIERS[0]; onClose: () => void }) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[500px]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button onClick={onClose} disabled={!form.name || !form.phone}
...
export default function SuppliersPage() {
  // TODO: const { data: suppliers = [] } = useSuppliers()
  const suppliers = MOCK_SUPPLIERS
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState<typeof MOCK_SUPPLIERS[0] | null>(null)
  const [formModal, setFormModal] = useState<typeof MOCK_SUPPLIERS[0] | 'new' | null>(null)
