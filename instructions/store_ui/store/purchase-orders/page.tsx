...
import { usePurchaseOrders, useCreatePurchaseOrder, useUpdatePOStatus, useAutoGeneratePO } from '@/hooks/useStore'
...
// ── Mock data — TODO: replace with usePurchaseOrders() ──────────────────────
const MOCK_POS: PurchaseOrder[] = [
  {
    id:'po1', poNumber:'PO-2025-003', status:'DRAFT',
    supplier:{ id:'s1', name:'Morogoro General Supplies', phone:'+255712000001', isActive:true },
    items:[
      { id:'poi1', item:{ id:'9',  name:'Shower Gel (50ml)',   unit:'PCS',  } as any, quantityOrdered:60, quantityReceived:0, unitCost:1200, totalCost:72000 },
      { id:'poi2', item:{ id:'10', name:'Toilet Soap (bar)',   unit:'PCS',  } as any, quantityOrdered:40, quantityReceived:0, unitCost:800,  totalCost:32000 },
      { id:'poi3', item:{ id:'12', name:'Garbage Bags (roll)', unit:'ROLL', } as any, quantityOrdered:18, quantityReceived:0, unitCost:2000, totalCost:36000 },
    ],
    totalAmount:140000, notes:'Urgent — items below minimum stock',
    expectedDelivery: new Date(Date.now()+86400000*2).toISOString(),
    createdBy:{ id:'u1', fullName:'Rehema Khamis' },
    createdAt: new Date(Date.now()-3600000).toISOString(),
    updatedAt: new Date(Date.now()-3600000).toISOString(),
  },
  {
    id:'po2', poNumber:'PO-2025-002', status:'RECEIVED',
    supplier:{ id:'s2', name:'Tanzania Breweries Ltd', phone:'+255712000002', isActive:true },
    items:[
      { id:'poi4', item:{ id:'1', name:'Serengeti Beer (500ml)',  unit:'BOTTLE' } as any, quantityOrdered:48, quantityReceived:48, unitCost:1800, totalCost:86400 },
      { id:'poi5', item:{ id:'3', name:'Coca Cola (300ml)',       unit:'BOTTLE' } as any, quantityOrdered:60, quantityReceived:60, unitCost:600,  totalCost:36000 },
    ],
    totalAmount:122400, notes:undefined,
  ...
  {
    id:'po3', poNumber:'PO-2025-001', status:'SENT_TO_SUPPLIER',
    supplier:{ id:'s3', name:'Karibu Textiles', phone:'+255712000003', isActive:true },
    items:[
      { id:'poi6', item:{ id:'7', name:'Bath Towel (Large)', unit:'PCS' } as any, quantityOrdered:20, quantityReceived:0, unitCost:8000, totalCost:160000 },
      { id:'poi7', item:{ id:'8', name:'Bed Sheet (King)',   unit:'PCS' } as any, quantityOrdered:12, quantityReceived:0, unitCost:15000,totalCost:180000 },
    ],
    totalAmount:340000, notes:'Monthly linen restock',
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[600px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-3">
            ...
            {next && (
              <button onClick={onClose}
                className="flex-1 h-10 bg-[#2563EB] hover:bg-[#1D4ED8] text-white rounded-lg text-[13px] font-semibold transition-colors">
                {next.label}
              </button>
            )}
          </div>
...
function NewPOModal({ onClose }: { onClose: () => void }) {
  ...
  const SUPPLIERS = [
    { id:'s1', name:'Morogoro General Supplies' },
    { id:'s2', name:'Tanzania Breweries Ltd'    },
    { id:'s3', name:'Karibu Textiles'           },
  ]
  const ITEMS_LIST = [
    { id:'1',  name:'Serengeti Beer (500ml)',  unitCost:1800 },
    { id:'3',  name:'Coca Cola (300ml)',        unitCost:600  },
    { id:'4',  name:'Mineral Water (500ml)',    unitCost:400  },
    { id:'9',  name:'Shower Gel (50ml)',        unitCost:1200 },
    { id:'10', name:'Toilet Soap (bar)',        unitCost:800  },
    { id:'11', name:'Toilet Paper (roll)',      unitCost:500  },
    { id:'12', name:'Garbage Bags (roll)',      unitCost:2000 },
  ]
  ...
  const updateItem = (i: number, k: string, v: string) => {
    setItems(prev => prev.map((item, j) => {
      ...
      if (k === 'itemId') {
        const found = ITEMS_LIST.find(x => x.id === v)
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[600px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-5">
                    ...
                    <tr key={i} className="border-b border-gray-50 last:border-0">
                      <td className="px-3 py-2">
                        <select className={cn(INPUT, 'h-9 text-[12px]')} value={item.itemId}
                          ...
                          {ITEMS_LIST.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}
                        </select>
                      </td>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onClose} disabled={!supplierId || items.every(i => !i.itemId)}
...
export default function PurchaseOrdersPage() {
  // TODO: const { data: orders = [] } = usePurchaseOrders()
  const orders = MOCK_POS
